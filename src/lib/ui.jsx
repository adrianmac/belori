...
export const PrimaryBtn = ({label,onClick,style={},className='',colorScheme='primary',children,disabled,loading,...rest}) => (
  <button onClick={onClick} className={`btn-solid ${className}`} data-color={colorScheme} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, ...style }} disabled={disabled || loading} aria-busy={loading} {...rest}>
    {loading && <SpinnerIcon />}
    {label||children}
  </button>
);
export const GhostBtn = ({label,onClick,style={},className='',colorScheme='primary',children,disabled,loading,...rest}) => (
  <button onClick={onClick} className={`btn-ghost ${className}`} data-color={colorScheme} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, ...style }} disabled={disabled || loading} aria-busy={loading} {...rest}>
    {loading && <SpinnerIcon />}
    {label||children}
  </button>
);
