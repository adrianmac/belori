      ...
      {alerts.length === 0 ? (
        ...
        return (
          <button key={alert.id} onClick={() => handleClick(alert)}
            style={{ display: 'flex', width: '100%', textAlign: 'left', background: 'transparent', border: 'none', alignItems: 'flex-start', gap: 10, padding: '11px 14px', cursor: 'pointer', borderBottom: i < alerts.length - 1 ? `1px solid ${C.border}` : 'none', transition: 'background 0.1s' }}
