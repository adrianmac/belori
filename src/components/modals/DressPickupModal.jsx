...
export const DressPickupModal = ({ dress, onClose, onUpdate }) => {
  ...
  return (
    <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: C.white, borderRadius: 16, width: 460, maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ padding: '18px 24px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 16, fontWeight: 600, color: C.ink }}>Mark picked up</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 20, color: C.gray, cursor: 'pointer' }} aria-label="Close modal">×</button>
        </div>
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <div><div style={LBL}>Pickup date</div><input type="date" value={pickupDate} onChange={e => setPickDate(e.target.value)} style={inputSt} /></div>
            <div><div style={LBL}>Return due</div><input type="date" value={returnDate} onChange={e => setRetDate(e.target.value)} style={inputSt} /></div>
          </div>
          <div><div style={LBL}>Deposit collected ($)</div><input type="number" value={depositAmt} onChange={e => setDepositAmt(Number(e.target.value))} style={inputSt} /></div>
          ...
          {warn && <div style={{ fontSize: 11, color: 'var(--color-warning)', background: 'var(--bg-warning)', padding: '8px 12px', borderRadius: 7 }}>⚠ We recommend confirming ID and agreement before releasing the dress.</div>}
        </div>
